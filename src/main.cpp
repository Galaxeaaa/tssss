...
int main(int argc, char **argv)
{
	...
	Shader sHaarTest("shader/HaarTest.vs.glsl", "shader/HaarTest.fs.glsl");
	...
	// ------------------------------
	GLuint gBuffer;
	GLuint gPosition, gNormal, gAlbedoSpec, gDepth, dbgImage;
	GLuint fBuffer;
	GLuint fDepth;
	GLuint haar_texture, haar_radiance_map, *haar_kernels;
	haar_kernels = new GLuint[haar::height * haar::width];
	...
	else if (mode == RenderingMode::HAAR)
	{
		// haar_texture = TextureFromFile(std::filesystem::current_path().string().c_str(), "resource/backpack/textures/Scene_-_Root_baseColor.jpeg");

		// glGenTextures(1, &haar_radiance_map);
		// glBindTexture(GL_TEXTURE_2D, haar_radiance_map);
		// glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, haar::width, haar::height, 0, GL_RGB, GL_FLOAT, NULL);
		// glBindImageTexture(0, haar_radiance_map, 0, GL_FALSE, 0, GL_READ_WRITE, GL_R32F);

		// glGenTextures(haar::width * haar::height, haar_kernels);
		// for (auto i = 0; i < haar::width * haar::height; i++)
		// {
		// 	glBindTexture(GL_TEXTURE_2D, haar_kernels[i]);
		// 	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, haar::width, haar::height, 0, GL_RGB, GL_FLOAT, NULL);
		// }
	}
	// dbg image
	...
	// - Buffer of visible light indices
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (deltaTime >= 1)
		{
			glfwSetWindowTitle(window, (to_string(nFrame) + " " + to_string(tF1) + "ms " + to_string(tF2) + "ms " + to_string(tF3) + "ms").c_str());
			nFrame = 0;
		...
		else if (mode == RenderingMode::HAAR)
		{
			...
			sHaarTest.setVec3("viewPos", camera.Position);
			sHaarTest.setMat4("projection", projection);
			sHaarTest.setMat4("view", view);
			model = glm::mat4(1.0f);
			model = glm::translate(model, glm::vec3(4.5, -0.5, 4.5));
			model = glm::scale(model, glm::vec3(1.0f));
			sHaarTest.setMat4("model", model);
			backpack.Draw(sHaarTest);
		}

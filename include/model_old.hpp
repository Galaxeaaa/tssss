...
private:
	...
	void processNode(aiNode *node, const aiScene *scene, const aiMatrix4x4 &transformation)
	{
		...
		{
			processNode(node->mChildren[i], scene, node->mTransformation.Transpose() * transformation);
		}
	}
	...
	Mesh processMesh(aiMesh *mesh, const aiScene *scene, const aiMatrix4x4 &transformation)
	{
		...
		// glm::mat4 mat(transformation.a1, transformation.a2, transformation.a3, transformation.a4, 
		...
		glm::mat4 mat(transformation.a1, transformation.b1, transformation.c1, transformation.d1, 
		...
		for (unsigned int i = 0; i < mesh->mNumVertices; i++)
		{
			...
			glm::vec4 Position4 = mat * glm::vec4(vector, 1.0);
			vertex.Position = glm::vec3(Position4) / Position4.w;
			// normals
			if (mesh->HasNormals())
			{
				...
				glm::mat4 nmat = glm::inverse(glm::transpose(mat));
				vertex.Normal = glm::vec3(nmat * glm::vec4(vector, 0.0));
			}
			// texture coordinates
